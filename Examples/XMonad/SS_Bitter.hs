...
new l wids m
  ...
  where (seen, unseen)
        ...
        (cur : visi)
          = let _x0 (i, s, sd) = return (Screen i s sd)
                _x0 _ = fail "..."
              in (>>=) (zip3 seen (enumFrom 0) m) _x0
...
view i s
  | (==) i (currentTag s) = s
  | Just x <- L.find ((.) (\ _x4 -> (==) i _x4) ((.) tag workspace))
                (visible s)
    =
    s{current = x,
      visible =
        (:) (current s) (L.deleteBy (equating screen) x (visible s))}
  | Just x <- L.find ((.) (\ _x5 -> (==) i _x5) tag) (hidden s) =
...
greedyView w ws
  ...
  | otherwise = ws
  where wTag = (.) (\ _x6 -> (==) w _x6) tag
 
lookupWorkspace :: (Eq s) => s -> StackSet i l a s sd -> Maybe i
lookupWorkspace sc w
  = listToMaybe
      (let _x1 (Screen i s _) = (>>) (guard ((==) s sc)) (return (tag i))
           _x1 _ = fail "..."
         in (>>=) ((:) (current w) (visible w)) _x1)
 
with :: b -> (Stack a -> b) -> StackSet i l a s sd -> b
with dflt f
  = (.) (maybe dflt f) ((.) stack ((.) workspace current))
...
focusWindow w s
  ...
  | otherwise =
    ($) (fromMaybe s)
      (let _x2 n
             = ($) return
                 (until ((.) (\ _x7 -> (==) (Just w) _x7) peek) focusUp (view n s))
           _x2 _ = fail "..."
         in (>>=) (findTag w s) _x2)
...
ensureTags l allt st
  = et allt ((\\) (map tag (workspaces st)) allt) st
  where et [] _ s = s
        et (i : is) rn s | tagMember i s = et is rn s
        et (i : is) [] s
...
findTag a s
  = listToMaybe
      (let _x3 w = (>>) (guard (has a (stack w))) (return (tag w))
           _x3 _ = fail "..."
         in (>>=) (workspaces s) _x3)
  where has _ Nothing = False
        has x (Just (Stack t l r)) = elem x ((:) t ((++) l r))
...
delete' w s
  ...
  where removeFromWorkspace ws
          = ws{stack = (>>=) (stack ws) (filter (\ _x8 -> (/=) _x8 w))}
